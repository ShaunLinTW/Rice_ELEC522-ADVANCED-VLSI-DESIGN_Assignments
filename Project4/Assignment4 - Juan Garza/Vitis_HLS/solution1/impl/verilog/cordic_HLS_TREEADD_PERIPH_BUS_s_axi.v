...
// ==============================================================
`timescale 1ns/1ps
module cordic_HLS_TREEADD_PERIPH_BUS_s_axi
#(parameter
...
localparam
    ...
    ADDR_IER          = 7'h08,
    ADDR_ISR          = 7'h0c,
    ADDR_STATE_DATA_0 = 7'h10,
    ADDR_STATE_CTRL   = 7'h14,
    ADDR_THETA_DATA_0 = 7'h18,
    ADDR_THETA_CTRL   = 7'h1c,
    ADDR_X_DATA_0     = 7'h20,
    ADDR_X_CTRL       = 7'h24,
    ADDR_Y_DATA_0     = 7'h28,
    ADDR_Y_CTRL       = 7'h2c,
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_THETA_DATA_0)
            int_theta[15:0] <= (WDATA[31:0] & wmask) | (int_theta[15:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (t_ap_vld)
            int_t_ap_vld <= 1'b1;
        else if (ar_hs && raddr == ADDR_T_CTRL)
            int_t_ap_vld <= 1'b0; // clear on read
    end
end
